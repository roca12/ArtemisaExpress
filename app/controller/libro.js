...
class LibroController {
  ...
  async actualizarLibro(req, res) {
    try{
      ...
      const libro = await this.service.actualizarLibro(id, { titulo, archivoPdf, imagen });
      res.status(200).json(libro);
    }catch(err){
      if(err.code === "LIMIT_FILE_SIZE"){
        return res.status(413).json({ok:false, message:"El archivo supera los 10MB permitidos"});
      }
  ...
  async eliminarLibro(req, res) {
    try{
      ...
      res.status(200).json({ok:true, message:"Archvio eliminado exitosamente."});
    }catch(err){
      res.status(err.statusCode||500).json({ok:false, message:err.message});
    }
  }
